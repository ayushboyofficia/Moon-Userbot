...
@Client.on_message(filters.command("love", hl) & filters.me)
async def send_love_pic(client: Client, message: Message):
    """Send a random love picture"""
    love_url = random.choice(LOVE_PICS)
    await message.reply_photo(love_url)

add_command_help(
    "love",
    [
        ["love", "Send a random romantic picture"],
    ],
)
